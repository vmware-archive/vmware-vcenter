...
#Getter method for nicorderpolicy
  def nicorderpolicy
    Puppet.debug "Retreiving nicorderpolicy associated with portgroup"
		begin
      existing_activenic = retrieve_portg_nicorder_policy.activeNic
      existing_standbynic = retrieve_portg_nicorder_policy.standbyNic
  ...
  def nicorderpolicy=(value)
	...
	def ipsettings
		...
		begin
			retipsettings = retrieve_ipsettings
			return retipsettings
  ...
  private
	...
	#retreive portg nicorder policy
	def retrieve_portg_nicorder_policy
		find_host
		portg = find_portgroup
		actualspec = portg.spec
		return actualspec.policy.nicTeaming.nicOrder
	end
	
 def retrieve_ipsettings
 	find_host
    @networksystem=@host.configManager.networkSystem
    vnics=@networksystem.networkInfo.vnic
