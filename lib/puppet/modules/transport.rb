  ...
  module Transport
    ...
    def self.retrieve(resource, catalog, provider)
      ...
      unless transport = find(name, provider)
        transport = Puppet::Modules::Transport::const_get(provider.capitalize).new(options)
        transport.connect
        @@instances << transport
      end
    ...
    def self.find(name, provider)
      @@instances.find{ |x| x.is_a? Puppet::Modules::Transport::const_get(provider.capitalize) and x.name == name }
    end
  end
end
